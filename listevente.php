...
<head>
    ...
    <title>Liste des ventes — Plateforme ENEAM</title>
    <style type="text/css">
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .montant { color: #f48fb1; font-weight: 700; font-size: 14px; }
        .date-badge { font-size: 12px; color: rgba(255,255,255,0.5); }
...
<div class="page">
    ...
    $requete = "SELECT commande.id_comm, commande.date, commande.montant,
                       client.nom, client.prenom, client.ville,
                       article.design, contenir.qté_comm
                FROM commande
                INNER JOIN client   ON commande.id_client   = client.id_client
                INNER JOIN contenir ON commande.id_comm = contenir.id_comm
    ...
    <div class="page-entete">
        <div class="page-titre">Liste des <span>Ventes</span></div>
        <div class="compteur"><?php echo $nbvente; ?> vente(s)</div>
    </div>

    <?php if ($nbvente == 0) { ?>
        <div class="tableau-boite"><p class="msg-vide">Aucune vente enregistrée.</p></div>
    <?php } else { ?>
    ...
    <div class="tableau-boite">
        <table>
            ...
            {
                echo "<tr>";
                echo "<td><span class='badge-id'>#" . $ligne['id_comm'] . "</span></td>";
                echo "<td><span class='date-badge'>" . $ligne['date'] . "</span></td>";
                echo "<td>" . $ligne['nom'] . " " . $ligne['prenom'] . "</td>";
                echo "<td>" . $ligne['ville'] . "</td>";
                echo "<td>" . $ligne['design'] . "</td>";
                echo "<td>" . $ligne['qté_comm'] . "</td>";
                echo "<td><span class='montant'>" . number_format($ligne['montant'],0,',',' ') . "</span></td>";
                echo "</tr>";
            }
            ?>
        </table>
